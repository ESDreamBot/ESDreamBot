...
if (message.content === '=invite') {
      message.author.send('https://discordapp.com/oauth2/authorize?client_id=546269855716737025&permissions=0&scope=bot|  تفضل ربط البوت     ').catch(e => console.log(e.stack));
...
client.on('message',async message => {
  ...
  if(!args[2]) {
    if(mention.bot) return message.channel.send(`**:heavy_multiplication_x:| ${message.content.split(' ')[1]} لم يتم العثور على**`);
    message.channel.send(`**${message.author.username}, your :credit_card: balance is \`\`${userData.credits}\`\`.**`)return;
  }
